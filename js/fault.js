...
const FaultChecker = (() => {
  ...
  function render(container) {
    ...
    const defVocExp = sr ? (sr.panel.Voc * sr.n_mod).toFixed(1) : '';
    const defIscExp = sr ? sr.panel.Isc.toFixed(2) : '';
    ...
    container.innerHTML = `
      <div class="page">
        ...
        <div class="card">
          ...
          ${sr ? `<div class="info-box">Pre-filled from Sizing Calculator: ${sr.panel.manufacturer} ${sr.panel.model}, ${sr.n_mod} mod/string.</div>` : ''}
          ...
          <div class="form-row cols-2">
            <div class="form-group">
              ...
              <input class="form-input" id="fc-nmod" type="number" value="${defNmod}" />
            </div>
            <div class="form-group">
              <label class="form-label">Expected String Voc (V)</label>
              <input class="form-input" id="fc-voc-exp" type="number" step="0.1" value="${defVocExp}" />
              <div class="form-hint">STC: n_mod × panel Voc. Leave blank to auto-calc.</div>
            </div>
            <div class="form-group">
              <label class="form-label">Expected Isc (A)</label>
              <input class="form-input" id="fc-isc-exp" type="number" step="0.01" value="${defIscExp}" />
    ...
    container.querySelector('#fc-bulk-btn').addEventListener('click', () => {
      const n = parseInt(prompt('How many strings to add?', '8'));
      if (!isNaN(n) && n > 0) {
  ...
  function _autofill(container) {
    ...
    if (!panel) return;
    const n = parseInt(container.querySelector('#fc-nmod').value) || 20;
  ...
  function _detect(container) {
    ...
    const n_mod    = parseInt(container.querySelector('#fc-nmod').value) || 20;
    ...
    if (isNaN(Voc_exp) || isNaN(Isc_exp)) {
      App.toast('Enter expected Voc and Isc values', 'error'); return;
    }
    ...
    const results = _strings.map(s => {
      ...
      const fault = PVCalc.detectFault(Voc, Isc, Voc_exp, Isc_exp, n_mod, panel || { Voc: Voc_exp / n_mod, Isc: Isc_exp });
      return { label: s.label, Voc, Isc, ...fault };
    });

    const valid = results.filter(r => !r.skipped);
    if (!valid.length) { App.toast('No valid measurements entered', 'error'); return; }
      ...
      <div class="card">
        ...
        <div style="overflow-x:auto">
          <table class="status-table">
            ...
            <tbody>
              ${results.map(r => {
                if (r.skipped) return `<tr><td><em>${r.label}</em></td><td colspan="6" class="text-muted">— no data —</td></tr>`;
        ...
        ${valid.filter(r => r.severity !== 'ok').map(r => `
          <div class="${r.severity === 'fault' ? 'danger-box' : 'info-box'}" style="margin-bottom:8px">
            <strong>${r.label} — ${r.fault}</strong><br>
            <span style="font-size:0.82rem">${r.detail}</span>
          </div>
